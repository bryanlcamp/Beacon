...
export class FormControls extends EventEmitter {
  ...
  validateField(fieldName, value) {
    ...
    if (element) {
      ...
      // Update adorner for message count based on threshold
      if (fieldName === 'messageCount') {
        this.updateMessageCountAdorner(element, value);
      }
  ...
  /**
   * Update message count adorner based on value and threshold
   * @param {HTMLElement} element - Form element
   * @param {*} value - Current value
   * @private
   */
  updateMessageCountAdorner(element, value) {
    const messageCount = parseInt(value) || 0;
    const spinnerControl = element.closest('.spinner-control');
    
    if (spinnerControl) {
      // Remove existing adorner
      spinnerControl.removeAttribute('data-adorner');
      
      // Apply adorner based on threshold from config
      if (messageCount < CONSTANTS.MESSAGE_COUNT.RED_ADORNER_THRESHOLD) {
        spinnerControl.setAttribute('data-adorner', 'red');
      } else if (messageCount >= CONSTANTS.MESSAGE_COUNT.RED_ADORNER_THRESHOLD) {
        spinnerControl.setAttribute('data-adorner', 'green');
      }
    }
    
    // Update generate button state when message count changes
  ...
  /**
   * Update generate button state based on symbols and validation
   */
  updateGenerateButtonState() {
    ...
    const messageCountValid = this.isMessageCountValid();
    
    const isReady = symbolCount > 0 && 
    ...
    // Update button text and state based on validation priority
    if (!messageCountValid) {
      this.setGenerateButtonState('invalid-message-count');
      ...
      this.setGenerateButtonState('validation-error');
    } else {
  ...
  /**
   * Check if message count meets validation rules
   * @returns {boolean} True if message count is valid
   * @private
   */
  isMessageCountValid() {
    const messageCountInput = this.getElement('messageCount');
    if (!messageCountInput) return false;
    
    const messageCount = parseInt(messageCountInput.value) || 0;
    return messageCount >= CONSTANTS.MESSAGE_COUNT.RED_ADORNER_THRESHOLD;
  }
  ...
  setGenerateButtonState(state) {
    ...
    const stateConfig = {
      ...
      'invalid-message-count': {
        text: 'Minimum 1,000 Messages Required',
        disabled: true,
        spinner: false
      },
      'validation-error': {
