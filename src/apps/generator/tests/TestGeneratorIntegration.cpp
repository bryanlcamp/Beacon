...
private:
  ...
  "Wave": {
    ...
    "WaveAmplitudePercent": 200.0
  },
  "Burst": {
    "Enabled": true,
    "BurstIntensityPercent": 400.0,
    "BurstFrequencyMs": 15000
  },
  "Symbols": [
    {
      "SymbolName": "AAPL",
...
TEST_F(TestGeneratorIntegration, MultipleExchangeTypes) {
    ...
    std::vector<std::string> exchanges = {"nsdq", "cme", "nyse"};
...
TEST_F(TestGeneratorIntegration, WaveAndBurstGeneration) {
    // Test that wave and burst configurations affect generation
    
    ConfigProvider configProvider("", "integration_output.csv");
    ASSERT_TRUE(configProvider.loadConfig("integration_test_config.json"));
    configProvider.setCsvMode(true);
    
    const auto& waveConfig = configProvider.getWaveConfig();
    const auto& burstConfig = configProvider.getBurstConfig();
    
    EXPECT_EQ(waveConfig.WaveDurationMs, 30000);
    EXPECT_EQ(waveConfig.WaveAmplitudePercent, 200.0);
    EXPECT_TRUE(burstConfig.Enabled);
    EXPECT_EQ(burstConfig.BurstIntensityPercent, 400.0);
    EXPECT_EQ(burstConfig.BurstFrequencyMs, 15000);
    
    MessageGenerator generator(configProvider);
    EXPECT_NO_THROW(generator.generateMessages("integration_output.csv", 500, "integration_test_config.json"));
    
    // Verify file was created successfully with wave/burst enabled
    EXPECT_TRUE(std::filesystem::exists("integration_output.csv"));
    EXPECT_GT(std::filesystem::file_size("integration_output.csv"), 0);
}
